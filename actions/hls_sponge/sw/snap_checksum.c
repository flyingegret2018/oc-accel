...
static void usage(const char *prog)
{
	printf("Usage: %s [-h] [-v, --verbose] [-V, --version]\n"
	       ...
	       "./snap_checksum -vv -t2500 -mSPONGE -N -cSPEED -n1 -f65536\n"
	       "\n"
	       "Generation of 65536*128/65536 = 128 calls :\n"
	       "./snap_checksum -vv -t2500 -mSPONGE -N -cSPEED -n128 -f65536\n"
	       "\n"	       
	       "./snap_checksum -vv -t2500 -mSPONGE -N -cSPEED -n4096 -f65536\n"
	       "\n"
	       "Generation of 65536*1/4 = 16384 calls ie 1 call every 4 calls until 65536...\n"
	       "./snap_checksum -vv -t2500 -mSPONGE -N -cSPEED -n1 -f4\n"
	       "\n"
	       "to run tests SHA3 or/and SHAKE :\n"
	       "./snap_checksum -mSPONGE -N -t800 -cSHA3\n"
	       "./snap_checksum -mSPONGE -N -t800 -cSHAKE\n"
	       "./snap_checksum -mSPONGE -N -t800 -cSHA3_SHAKE\n"
	       "\n",
	       prog);
}
		       ...
		       snap_action_flag_t action_irq)
{
	...
	if (action == NULL) {
		fprintf(stderr, "err (%d): Card: %d failed to attach action 0x%x: %s\n",
			errno, card_no, ACTION_TYPE, strerror(errno));
		goto out_error1;
	}
